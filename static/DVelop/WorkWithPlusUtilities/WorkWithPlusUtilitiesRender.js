...
function DVelop_WorkWithPlusUtilities() {
	...
	this.FloatingLabels_AnimateLabel = function (elem, i) {
		...
		var addReadOnly = (!$(elem).is(":visible")) && $(elem).parents(".form-group").find('span').is(":visible") && (elem.id == null || !elem.id.match(/_GXI$/));
		...
		$(elem).parents(".form-group").toggleClass("FloatingLabelEmpty", addEmptyVal);
		$(elem).parents(".form-group").toggleClass("FloatingLabelReadonly", addReadOnly);
	}

	//ApplyBootstrapSelect - Start
	this.ApplyBootstrapSelect = function () {
		var thisC = this;
		$('.ConvertToDDO').each(function (g, h) {
			if ($(h).is(":visible") && !$(h).hasClass("selectpicker")) {
				$(h).addClass("selectpicker").selectpicker({
					noneSelectedText: "",
					showContent: false
				});
